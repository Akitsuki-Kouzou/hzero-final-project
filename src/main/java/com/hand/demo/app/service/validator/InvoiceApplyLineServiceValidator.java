...
@ImportValidators({@ImportValidator(templateCode = "46324.IAL.IMPORT")})
public class InvoiceApplyLineServiceValidator extends BatchValidatorHandler {
    @Override
    public boolean validate(List<String> data) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            if (isEmpty(invoiceApplyLine.getTaxClassificationNumber())) {
                addErrorMsg(i,"Tax Classification is mandatory");
                isValid = false;
            }
            ...
            if (isEmpty(invoiceApplyLine.getTaxRate())) {
                addErrorMsg(i,"Tax Rate Type is mandatory");
                isValid = false;
            }
        }
